...
export async function POST(request: Request) {
  try {
    ...
    let assessment = await db.prepare(
      "SELECT id, max_attempts FROM assessments WHERE active = 1 ORDER BY version DESC LIMIT 1",
    ).first() as { id: string; max_attempts: number } | null;
    if (!assessment) {
      ...
      await db.prepare(
        `INSERT INTO assessments
         ...
         VALUES (?, 'Trainora Core Quality & Integrity', 'All disciplines', 1, 25, 80, 2, ?, 1, ?, ?)`,
      ).bind(assessmentId, JSON.stringify(coreQualityQuestions), now(), now()).run();
    ...
    return Response.json({ attemptId, status: "assigned", maxAttempts: assessment.max_attempts });
  } catch (error) {
